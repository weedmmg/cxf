...
public final class ServerConnectionManager implements ConnectionManager {
    ...
    public void init() {
        if (heartbeatCheck) {
            ...
            this.timer = new HashedWheelTimer(new NamedThreadFactory(ThreadNames.T_CONN_TIMER), tickDuration, TimeUnit.MILLISECONDS, ticksPerWheel);
        }
    }
    ...
    public Connection get(Channel channel) {
        return connections.getOrDefault(channel.id().toString(), DEFAULT).get();
    }

    @Override
    public void add(Connection connection) {
        connections.putIfAbsent(connection.getChannel().id().toString(), holderFactory.create(connection));
    }

    @Override
    public Connection removeAndClose(Channel channel) {
        ConnectionHolder holder = connections.remove(channel.id().toString());
        if (holder != null) {
    ...
    private class HeartbeatCheckTask implements ConnectionHolder, TimerTask {
        ...
        public void run(Timeout timeout) throws Exception {
            ...
            if (connection == null || !connection.isConnected()) {
                ...
                Logs.HB.info(String.format("heartbeat timeout times={}, connection disconnected, conn={}", timeoutTimes, connection));
                return;
            }
            ...
            if (connection.isReadTimeout()) {
                if (++timeoutTimes > Integer.valueOf((PropertiesUtil.getValue("max-hb-timeout-times")))) {
                    connection.close();
                    Logs.HB.warn(String.format("client heartbeat timeout times={}, do close conn={}", timeoutTimes, connection));
                    return;
                } else {
                    Logs.HB.info(String.format("client heartbeat timeout times={}, connection={}", timeoutTimes, connection));
                }
            } else {
                timeoutTimes = 0;
            }
    ...
    @Override
    public Connection getById(String channelId) {
