...
public final class NettyConnection implements Connection, ChannelFutureListener {
    ...
    @Override
    public ChannelFuture send(Object packet, final ChannelFutureListener listener) {
        if (channel.isActive()) {
            ...
            // 阻塞调用线程还是抛异常？
            // return channel.newPromise().setFailure(new
            // RuntimeException("send data too busy"));
            ...
            /*
             * if (listener != null) { channel.newPromise()
             * .addListener(listener) .setFailure(new
             * RuntimeException("connection is disconnected")); }
             */
            return this.close();
        }
    }
    ...
    public void operationComplete(ChannelFuture future) throws Exception {
        if (future.isSuccess()) {
            ...
            LOGGER.debug("connection send msg error", future.cause());
            LOGGER.debug("connection send msg error={}, conn={}", future.cause().getMessage(), this);
        }
    }
