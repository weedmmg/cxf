...
public final class ConnectionServer extends NettyTCPServer {
    ...
    private ServerTCPHandler channelHandler;
    ...
    protected void initOptions(ServerBootstrap b) {
         ...
         * 这个buffer是用在netty向channel
         * write数据的时候，有个buffer缓冲，这样可以提高网络的吞吐量(每个channel有一个这样的buffer)。
         * 初始大小是32(32个元素，不是指字节)，但是如果超过32就会翻倍，一直增长。
         * 大部分时候是没有什么问题的，但是在碰到对端非常慢(对端慢指的是对端处理TCP包的速度变慢
         * ，比如对端负载特别高的时候就有可能是这个情况)的时候就有问题了，
         * 这个时候如果还是不断地写数据，这个buffer就会不断地增长，最后就有可能出问题了(我们的情况是开始吃swap，最后进程被linux
         * killer干掉了)。
        ...
        b.childOption(ChannelOption.WRITE_BUFFER_WATER_MARK,
                new WriteBufferWaterMark(Integer.valueOf(prop.getProperty("connect-server.low")), Integer.valueOf(prop.getProperty("connect-server.high"))));
    }
