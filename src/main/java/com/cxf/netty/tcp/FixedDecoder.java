...
public class FixedDecoder extends ByteToMessageDecoder {
     ...
     */
    public FixedDecoder(int maxFrameLength, int lengthFieldOffset, int lengthFieldLength) {
     ...
     */
    public FixedDecoder(int maxFrameLength, int lengthFieldOffset, int lengthFieldLength, int lengthAdjustment, int initialBytesToStrip) {
     ...
     *            exceed <tt>maxFrameLength</tt> regardless of whether the
     *            entire frame has been read. If <tt>false</tt>, a
     *            {@link TooLongFrameException} is thrown after the entire frame
     *            that exceeds <tt>maxFrameLength</tt> has been read.
     */
    public FixedDecoder(int maxFrameLength, int lengthFieldOffset, int lengthFieldLength, int lengthAdjustment, int initialBytesToStrip, boolean failFast) {
     ...
     *            that exceeds <tt>maxFrameLength</tt> has been read.
     */
    public FixedDecoder(ByteOrder byteOrder, int maxFrameLength, int lengthFieldOffset, int lengthFieldLength, int lengthAdjustment, int initialBytesToStrip, boolean failFast) {
        if (byteOrder == null) {
        ...
        if (lengthFieldOffset > maxFrameLength - lengthFieldLength) {
            throw new IllegalArgumentException("maxFrameLength (" + maxFrameLength + ") " + "must be equal to or greater than " + "lengthFieldOffset (" + lengthFieldOffset + ") + "
    ...
    protected Object decode(ChannelHandlerContext ctx, ByteBuf in) throws Exception {
        ...
        if (frameLength < lengthFieldEndOffset) {
            Logs.TCP.error("lengthFieldEndOffset 1");
            in.skipBytes(lengthFieldEndOffset);
        ...
        if (frameLength > maxFrameLength) {
            Logs.TCP.error("maxFrameLength 2");
        ...
        if (in.readableBytes() < frameLengthInt) {
            Logs.TCP.error("in.readableBytes() 3");
            // in.skipBytes(in.readableBytes()).retain();
            throw new CorruptedFrameException("Adjusted frame length (" + frameLength + ") is large " + "than readableBytes: " + in.readableBytes());
        ...
        if (initialBytesToStrip > frameLengthInt) {
            Logs.TCP.error("initialBytesToStrip 4");
    ...
    protected long getUnadjustedFrameLength(ByteBuf buf, int offset, int length, ByteOrder order) {
        ...
        // Logs.TCP.error("data len:" + frameLength);
        return frameLength;
    }
     ...
     * current {@link #decode(ChannelHandlerContext, ByteBuf)} call returns, you
     * can even avoid memory copy by returning the sliced sub-region (i.e.
     ...
     * {@link ObjectDecoder} to see how this method is overridden to avoid
     * memory copy.
     */
